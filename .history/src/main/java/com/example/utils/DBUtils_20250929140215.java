...
public class DBUtils {
    ...
    public static void initDB() {
        ...
        try {
            ...
            if (files.isEmpty()) {
                ...
                // Thêm file với size/hash thực tế từ thư mục resources
                insertFileFromResource("test.txt", expectedHash);
                insertFileFromResource("sample.txt", null);
                insertFileFromResource("large_document.txt", null);
                insertFileFromResource("system_admin.sh", null);
                insertFileFromResource("DistributedComputingExample.java", null);
    ...
    private static void insertFileFromResource(String name, String overrideHash) {
        try {
            String basePath = System.getProperty("user.dir") + "/src/main/resources/";
            File f = new File(basePath + name);
            if (!f.exists()) {
                System.err.println("Resource file not found: " + f.getAbsolutePath());
                return;
            }
            long size = f.length();
            String hash = (overrideHash != null && !overrideHash.isEmpty())
                    ? overrideHash
                    : FileUtils.calculateHash(f.getAbsolutePath(), "SHA-256");
            insertFile(name, size, hash);
        } catch (Exception ex) {
            System.err.println("Failed to insert resource file " + name + ": " + ex.getMessage());
        }
    }
